...
public class FMessageTkk {
    ...
    public FMessageTkk(Object fMessage) {
        if (fMessage == null) throw new RuntimeException("Object fMessage is null");
        if (!FMessageTkk.TYPE.isInstance(fMessage))
    ...
    public static class Key {
        ...
        /**
         * The wrapped FMessageTkk instance associated with this key.
         */
        ...
        public Key(Object key) {
            ...
            var fmessage = TkkCore.getFMessageFromKey(key);
            if (fmessage != null) {
                this.fmessage = new FMessageTkk(fmessage);
            }
        }

        public Key(Object key, FMessageTkk fmessage) {
            this.thisObject = key;
            this.messageID = (String) XposedHelpers.getObjectField(key, "A01");
            this.isFromMe = XposedHelpers.getBooleanField(key, "A02");
            this.remoteJid = new UserJid(XposedHelpers.getObjectField(key, "A00"));
        ...
        public Key(String messageID, UserJid remoteJid, boolean isFromMe) {
            ...
            var key = XposedHelpers.newInstance(FMessageTkk.Key.TYPE, remoteJid.userJid, messageID, false);
            var fmessage = TkkCore.getFMessageFromKey(key);
            if (fmessage != null) {
                this.thisObject = key;
                this.fmessage = new FMessageTkk(fmessage);
            } else {
                key = XposedHelpers.newInstance(FMessageTkk.Key.TYPE, remoteJid.phoneJid, messageID, false);
                fmessage = TkkCore.getFMessageFromKey(key);
                if (fmessage != null) {
                    this.thisObject = key;
                    this.fmessage = new FMessageTkk(fmessage);
                }
    ...
    public static class UserJid {
        ...
        public UserJid(@Nullable String rawjid) {
            ...
            if (checkValidLID(rawjid)) {
                this.userJid = TkkCore.createUserJid(rawjid);
                this.phoneJid = TkkCore.getPhoneJidFromUserJid(this.userJid);
            } else {
                this.phoneJid = TkkCore.createUserJid(rawjid);
                this.userJid = TkkCore.getUserJidFromPhoneJid(this.phoneJid);
            }
        }
