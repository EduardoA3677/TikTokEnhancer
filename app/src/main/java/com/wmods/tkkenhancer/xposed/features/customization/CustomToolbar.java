...
public class CustomToolbar extends Feature {
    ...
    public void doHook() throws Exception {
        ...
        XposedHelpers.findAndHookMethod(TkkCore.getHomeActivityClass(classLoader), "onCreate", Bundle.class, methodHook);
        expirationAboutInfo();
    ...
    public void expirationAboutInfo() throws Exception {
        ...
        XposedHelpers.findAndHookMethod(TkkCore.getAboutActivityClass(classLoader), "onCreate", classLoader.loadClass("android.os.Bundle"),
                new XC_MethodHook() {
                    @SuppressLint("SetTextI18n")
                    @Override
                    protected void afterHookedMethod(MethodHookParam param) throws Throwable {
    ...
    public static class MethodHook extends XC_MethodHook {
        ...
        protected void afterHookedMethod(MethodHookParam param) throws Throwable {
            ...
            var name = TkkCore.getMyName();
            var bio = TkkCore.getMyBio();
            var window = (ViewGroup) homeActivity.getWindow().getDecorView();
            var clazz = TkkCore.getTabsPagerClass(homeActivity.getClassLoader());
