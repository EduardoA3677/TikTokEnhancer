...
import static com.wmods.tkkenhancer.xposed.features.others.MenuHome.menuItems;
...
import com.wmods.tkkenhancer.utils.RealPathUtil;
...
public class LiteMode extends Feature {
    ...
    private static void showDialogUriPermission(Activity activity) {
        new AlertDialogTkk(activity).setTitle(activity.getString(ResId.string.download_folder_permission))
    ...
    public void doHook() throws Throwable {
        ...
        XposedHelpers.findAndHookMethod(TkkCore.getHomeActivityClass(classLoader), "onCreate", Bundle.class, new XC_MethodHook() {
            @Override
            protected void afterHookedMethod(MethodHookParam param) throws Throwable {
                var activity = (Activity) param.thisObject;

                var wae = TkkCore.getPrivString("download_folder", null);
    ...
    public static String processDownloadResult(Activity activity, Intent intent) {
        ...
        TkkCore.setPrivString("download_folder", uri.toString());
    ...
    private void InsertDownloadFolderButton(Menu menu, Activity activity) {
        ...
        itemMenu.setOnMenuItemClickListener(item -> {
            ...
            AlertDialogTkk dialog = new AlertDialogTkk(activity);
