...
import static com.wmods.tkkenhancer.xposed.core.FeatureLoader.disableExpirationVersion;
...
import com.wmods.tkkenhancer.listeners.OnMultiClickListener;
...
import com.wmods.tkkenhancer.xposed.utils.AnimationUtil;
...
//        propsBoolean.put(3403, true);  // Enable Sticker Suggestion
        ...
        XposedBridge.hookAllMethods(clsCallEventCallback, "fieldstatsReady", new XC_MethodHook() {
            ...
            protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
                if (clsWamCall.isInstance(param.args[0])) {
                    ...
                    var userJid = new FMessageTkk.UserJid(XposedHelpers.callMethod(callinfo, "getPeerJid"));
    ...
    private void showCallInformation(Object wamCall, FMessageTkk.UserJid userJid) throws Exception {
        if (userJid.isGroup()) return;
        var sb = new StringBuilder();
        var contact = TkkCore.getContactName(userJid);
                            ...
                            for (int i = 0; i < reactionView.getChildCount(); i++) {
                                if (reactionView.getChildAt(i) instanceof TextView textView) {
                                    if (textView.getText().toString().contains(emoji)) {
                                        TkkCore.sendReaction("", param.args[2]);
                                        return;
                                    }
                                }
                            }
                        }
                        TkkCore.sendReaction(emoji, param.args[2]);
                    }
                };
    ...
    private void showOnline(boolean showOnline) throws Exception {
        ...
        XposedBridge.hookMethod(checkOnlineMethod, new XC_MethodHook() {
            ...
            protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
                ...
                if (userjid.isGroup()) return;
                var name = TkkCore.getContactName(userjid);
                ...
                Tasker.sendTaskerEvent(name, TkkCore.stripJID(jid), "contact_online");
            }
        });
    }
    ...
    private void hookProps() throws Exception {
        ...
        var dataUsageActivityClass = TkkCore.getDataUsageActivityClass(classLoader);
    ...
    private void hookSearchbar(String filterChats) throws Exception {
        ...
        XposedHelpers.findAndHookMethod(TkkCore.getHomeActivityClass(classLoader), "onPrepareOptionsMenu", Menu.class, new XC_MethodHook() {
            @Override
            protected void afterHookedMethod(MethodHookParam param) throws Throwable {
