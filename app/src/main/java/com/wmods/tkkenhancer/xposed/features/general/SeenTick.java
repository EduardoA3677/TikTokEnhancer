...
public class SeenTick extends Feature {
    ...
    private final Set<FMessageTkk> statuses = ConcurrentHashMap.newKeySet();
    private static Object mWaJobManager;
    private static Class<?> mSendReadClass;
    private static Method WaJobManagerMethod;
    private static FMessageTkk.UserJid currentJid;
    ...
    public void doHook() throws Throwable {
        ...
        TkkCore.addListenerActivity((activity, type) -> {
            if (activity.getClass().getSimpleName().equals("Conversation") && (type == TkkCore.ActivityChangeState.ChangeType.STARTED || type == TkkCore.ActivityChangeState.ChangeType.RESUMED)) {
                var jid = TkkCore.getCurrentUserJid();
        ...
        XposedBridge.hookMethod(setPageActiveMethod, new XC_MethodHook() {
            ...
            protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
                ...
                if (!FMessageTkk.TYPE.isInstance(object)) {
                    var fmessageField = ReflectionUtils.findFieldUsingFilter(object.getClass(), field -> FMessageTkk.TYPE.isAssignableFrom(field.getType()));
                    object = fmessageField.get(object);
                }
                var fMessage = new FMessageTkk(object);
                statuses.clear();
        ...
        if (ticktype == 1) {
            XposedBridge.hookMethod(viewButtonMethod, new XC_MethodHook() {
                ...
                protected void afterHookedMethod(MethodHookParam param) throws Throwable {
                    ...
                    var fMessageField = ReflectionUtils.getFieldByExtendType(viewStatusField.getDeclaringClass(), FMessageTkk.TYPE);
                    ...
                    if (fMessageObj == null) {
                        ...
                        fMessageField = ReflectionUtils.findFieldUsingFilterIfExists(instance.getClass(), field1 -> FMessageTkk.TYPE.isAssignableFrom(field1.getType()));
                    ...
                    public void onClick(MenuItem item, Object fragmentInstance, FMessageTkk fMessageWpp) {
                        try {
                            ...
                            for (int i = 0; i < listStatus.size(); i++) {
                                ...
                                if (!FMessageTkk.TYPE.isInstance(obj)) {
                                    var fieldFMessage = ReflectionUtils.getFieldByExtendType(obj.getClass(), FMessageTkk.TYPE);
                                    obj = fieldFMessage.get(obj);
                                }
                                var fMessage = new FMessageTkk(obj);
    ...
    private void hookViewOnceScreen(int ticktype) throws Exception {
        ...
        XposedBridge.hookMethod(menuMethod, new XC_MethodHook() {
            ...
            protected void afterHookedMethod(MethodHookParam param) throws Throwable {
                var messageField = ReflectionUtils.getFieldByExtendType(menuMethod.getDeclaringClass(), FMessageTkk.TYPE);
                Object fmessageObj = null;
                if (messageField != null) {
                    fmessageObj = messageField.get(param.thisObject);
                }
                if (fmessageObj == null) {
                    var keyField = ReflectionUtils.getFieldByExtendType(param.thisObject.getClass(), FMessageTkk.Key.TYPE);
                    if (keyField != null) {
                        var keyObj = keyField.get(param.thisObject);
                        fmessageObj = TkkCore.getFMessageFromKey(keyObj);
                    }
                }
                FMessageTkk fMessage = new FMessageTkk(fmessageObj);
        ...
        XposedHelpers.findAndHookMethod(TkkCore.getViewOnceViewerActivityClass(classLoader), "onCreateOptionsMenu", classLoader.loadClass("android.view.Menu"),
                new XC_MethodHook() {
                    @Override
                    protected void afterHookedMethod(MethodHookParam param) throws Throwable {
                        Menu menu = (Menu) param.args[0];
                        ...
                        item.setOnMenuItemClickListener(item1 -> {
                            CompletableFuture.runAsync(() -> {
                                var keyClass = FMessageTkk.Key.TYPE;
                                ...
                                var fMessage = new FMessageTkk.Key(keyMessage).getFMessage();
    ...
    private void hookOnSendMessages() throws Exception {
        ...
        XposedBridge.hookMethod(messageJobMethod, new XC_MethodHook() {
            ...
            protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
                ...
                var userJid = new FMessageTkk.UserJid(TkkCore.createUserJid(rawJid));
    ...
    private static void updateMessageStatusView(String rawJid, List<FMessageTkk> messages) {
    ...
    private void sendBlueTick(FMessageTkk.UserJid userJid) {
        CompletableFuture.runAsync(() -> {
            if (Objects.equals(userJid.getPhoneNumber(), Utils.getMyNumber()) || Objects.requireNonNullElse(userJid.getUserRawString(), "").contains("lid_me"))
                return;
            var messages = new ArrayList<FMessageTkk>();
    ...
    private void sendBlueTickMsg(FMessageTkk.UserJid userJid, ArrayList<FMessageTkk> messages) {
        if (messages.isEmpty())
            return;
        try {
            HashMap<FMessageTkk.UserJid, List<String>> groupedMap = new HashMap<>();
            for (FMessageTkk message : messages) {
    ...
    private void sendBlueTickStatus(FMessageTkk.UserJid currentJid) {
        CompletableFuture.runAsync(() -> {
            ...
            try {
                ...
                var userJidSender = TkkCore.createUserJid("status@broadcast");
    ...
    private void sendBlueTickMedia(FMessageTkk fMessage) {
        CompletableFuture.runAsync(() -> {
            try {
                var userJid = fMessage.getKey().remoteJid;
