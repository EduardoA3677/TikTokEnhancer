...
import com.wmods.tkkenhancer.adapter.MessageAdapter;
import com.wmods.tkkenhancer.views.NoScrollListView;
...
public class ShowEditMessage extends Feature {
                ...
                if (textView != null && !textView.getText().toString().contains("\uD83D\uDCDD")) {
                    ...
                    textView.setOnClickListener((v) -> {
                        try {
                            ...
                            var fMesage = new FMessageTkk(messageObj);
    ...
    private void showBottomDialog(ArrayList<MessageHistory.MessageItem> messages) {
        Objects.requireNonNull(TkkCore.getCurrentConversation()).runOnUiThread(() -> {
            var ctx = (Context) TkkCore.getCurrentConversation();

            var dialog = TkkCore.createBottomDialog(ctx);
            // NestedScrollView
