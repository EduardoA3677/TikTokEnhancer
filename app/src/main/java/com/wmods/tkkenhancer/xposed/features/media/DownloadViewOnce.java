...
public class DownloadViewOnce extends Feature {
    ...
    private static void downloadFile(FMessageTkk.UserJid userJid, File file) throws Exception {
        ...
        if (prefs.getBoolean("downloadviewonce", false)) {
            ...
            XposedBridge.hookMethod(menuMethod, new XC_MethodHook() {
                ...
                protected void afterHookedMethod(MethodHookParam param) throws Throwable {
                    var fmessageField = ReflectionUtils.getFieldByExtendType(param.thisObject.getClass(), FMessageTkk.TYPE);
                        ...
                        protected void afterHookedMethod(MethodHookParam param) throws Throwable {
                            ...
                            item.setOnMenuItemClickListener(item1 -> {
                                CompletableFuture.runAsync(() -> {
                                    ...
                                    var fmessage = new FMessageTkk.Key(keyMessageObj).getFMessage();
                                    var file = fmessage.getMediaFile();
                                    if (file == null) {
                                        Utils.showToast(Utils.getApplication().getString(ResId.string.download_not_available), 1);
                                        return;
