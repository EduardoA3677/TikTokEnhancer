...
import static com.wmods.tkkenhancer.xposed.features.general.LiteMode.REQUEST_FOLDER;
import static com.wmods.tkkenhancer.xposed.features.general.LiteMode.getDownloadsUri;
import static com.wmods.tkkenhancer.xposed.features.general.LiteMode.processDownloadResult;
...
public class ActivityController extends Feature {
    ...
    public static final String EXPORTED_ACTIVITY = TkkCore.getSettingsNotificationsActivityClass(ClassLoader.getSystemClassLoader()).getName();
    ...
    private static void processResultContact(Intent intent, Activity activity) {
        ...
        for (Object contactUserJid : listContacts) {
            var rawContacts = new FMessageTkk.UserJid(contactUserJid).getPhoneRawString();
    ...
    private static void contactController(Intent intent, Activity activity, Class<?> statusDistribution) throws IllegalAccessException, InstantiationException, InvocationTargetException {
        ...
        if (contacts != null) {
            for (String contact : contacts) {
                try {
                    Object jid = TkkCore.createUserJid(contact);
