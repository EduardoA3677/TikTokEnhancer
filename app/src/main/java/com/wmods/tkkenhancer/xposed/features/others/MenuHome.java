...
public class MenuHome extends Feature {
    ...
    private void InsertGhostModeOption(Menu menu, Activity activity, boolean newSettings) {
        ...
        if (!prefs.getBoolean("ghostmode", true)) {
            if (ghostmode) {
                TkkCore.setPrivBoolean("ghostmode", false);
        ...
        itemMenu.setOnMenuItemClickListener(item -> {
            new AlertDialogTkk(activity).setTitle(activity.getString(ResId.string.ghost_mode_s, (ghostmode ? "ON" : "OFF"))).
                    ...
                    .setNegativeButton(activity.getString(ResId.string.enable), (dialog, which) -> {
                        TkkCore.setPrivBoolean("ghostmode", true);
                        Utils.doRestart(activity);
                    }).show();
            return true;
    ...
    private void InsertDNDOption(Menu menu, Activity activity, boolean newSettings) {
        var dndmode = TkkCore.getPrivBoolean("dndmode", false);
        if (!prefs.getBoolean("show_dndmode", false)) {
            if (TkkCore.getPrivBoolean("dndmode", false)) {
                TkkCore.setPrivBoolean("dndmode", false);
                Utils.doRestart(activity);
            }
            return;
        }
            ...
            if (!dndmode) {
                new AlertDialogTkk(activity)
                        ...
                        .setPositiveButton(activity.getString(ResId.string.activate), (dialog, which) -> {
                            TkkCore.setPrivBoolean("dndmode", true);
                            Utils.doRestart(activity);
                        })
    ...
    private void InsertFreezeLastSeenOption(Menu menu, Activity activity, boolean newSettings) {
        var freezelastseen = TkkCore.getPrivBoolean("freezelastseen", false);
        if (!prefs.getBoolean("show_freezeLastSeen", true)) {
            if (freezelastseen) {
                TkkCore.setPrivBoolean("freezelastseen", false);
                Utils.doRestart(activity);
            }
            return;
        }
        ...
        item.setOnMenuItemClickListener(menuItem -> {
            if (!freezelastseen) {
                new AlertDialogTkk(activity)
                        ...
                        .setPositiveButton(activity.getString(ResId.string.activate), (dialog, which) -> {
                            TkkCore.setPrivBoolean("freezelastseen", true);
                            Utils.doRestart(activity);
                        })
                        .setNegativeButton(activity.getString(ResId.string.cancel), (dialog, which) -> dialog.dismiss())
                        .create().show();
    ...
    private void hookMenu() {
        XposedHelpers.findAndHookMethod(TkkCore.getHomeActivityClass(classLoader), "onCreateOptionsMenu", Menu.class, new XC_MethodHook() {
            @Override
