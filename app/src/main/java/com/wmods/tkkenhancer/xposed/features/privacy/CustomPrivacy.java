...
import com.wmods.tkkenhancer.adapter.CustomPrivacyAdapter;
...
public class CustomPrivacy extends Feature {
    ...
    public static JSONObject getJSON(String number) {
        ...
        return TkkCore.getPrivJSON(number + "_privacy", new JSONObject());
    }
    ...
    public void doHook() throws Throwable {
        ...
        if (type == 1) {
            var hooker = new TkkCore.ActivityChangeState() {
            ...
            TkkCore.addListenerActivity(hooker);
        } else if (type == 2) {
    ...
    private void showCustomPrivacyList(Activity activity, Class<?> contactClass, Class<?> groupClass) {
        ...
        SharedPreferences pprefs = TkkCore.getPrivPrefs();
        ...
        for (var key : maps.keySet()) {
            if (key.endsWith("_privacy")) {
                ...
                var userJid = new FMessageTkk.UserJid(number + (number.length() > 14 ? "@g.us" : "@s.whatsapp.net"));

                var contactName = TkkCore.getContactName(userJid);
        ...
        AlertDialogTkk builder = new AlertDialogTkk(activity);
    ...
    private void showPrivacyDialog(Activity activity, boolean isChat) {
        ...
        AlertDialogTkk builder = createPrivacyDialog(activity, userJid.getPhoneNumber());
        builder.show();
    }

    private FMessageTkk.UserJid getUserJid(Activity activity, boolean isChat) {
        if (isChat) {
            return new FMessageTkk.UserJid(ReflectionUtils.callMethod(chatUserJidMethod, activity));
        } else {
            return new FMessageTkk.UserJid(ReflectionUtils.callMethod(groupUserJidMethod, activity));
        }
    }

    private AlertDialogTkk createPrivacyDialog(Activity activity, String number) {
        AlertDialogTkk builder = new AlertDialogTkk(activity);
        builder.setTitle(ResId.string.custom_privacy);
    ...
    private void savePreferences(String number, String[] itemsKeys, boolean[] checkedItems) {
        try {
            ...
            TkkCore.setPrivJSON(number + "_privacy", jsonObject);
        } catch (Exception e) {
            Utils.showToast(e.getMessage(), Toast.LENGTH_SHORT);
        }
    }
