...
public class HideSeen extends Feature {
    ...
    protected static FMessageTkk.Key getKeyMessage(XC_MethodHook.MethodHookParam param, Object userJidObject, List<Pair<Integer, Class<? extends String>>> strings) {
        var keyObject = ReflectionUtils.getArg(param.args, FMessageTkk.Key.TYPE, 0);
        if (keyObject == null) {
            if (strings.size() < 2)
                return null;
            var idMessage = (String) param.args[strings.get(0).first];
            var userJid = new FMessageTkk.UserJid(userJidObject);
            return new FMessageTkk.Key(idMessage, userJid, false);
        } else {
            return new FMessageTkk.Key(keyObject);
        }
    }
    ...
    public void doHook() throws Exception {
        ...
        XposedBridge.hookMethod(SendReadReceiptJobMethod, new XC_MethodHook() {
            ...
            protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
                ...
                if (TextUtils.isEmpty(lid) || lid.contains("lid_me") || lid.contains("status_me"))
                    return;
                FMessageTkk.UserJid userJid = new FMessageTkk.UserJid(lid);
                ...
                if (userJid.isGroup()) {
                    ...
                    var customHideStatusView = CustomPrivacy.getJSON(TkkCore.stripJID(participant)).optBoolean("HideViewStatus", hidestatusview);
                ...
                if (isHide) {
                    for (String messageId : messageIds) {
                        var fmessage = new FMessageTkk.Key(messageId, userJid, false).getFMessage();
        ...
        XposedBridge.hookMethod(ReceiptMethod, new XC_MethodHook() {
            @Override
            protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
                if (TkkCore.getCurrentConversation() != TkkCore.getCurrentActivity()) return;
        ...
        XposedBridge.hookMethod(loadSenderPlayedBusiness, new XC_MethodHook() {
            ...
            protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
                ...
                if (set != null && !set.isEmpty()) {
                    var fMessage = new FMessageTkk(set.iterator().next());
                    var media_type = fMessage.getMediaType();  // 2 = voice note ; 82 = viewonce note voice; 42 = image view once; 43 = video view once
                    var isHide = false;
                    if ((hideonceseen || ghostmode) && fMessage.isViewOnce()) {
                        param.setResult(null);
